...
class AccountPaymentOrder(models.Model):
    ...
    def generate_payment_file(self):
        ...
        if self.payment_method_id.code == "Visa":
            card_brand_and_type = "DEBLIQC "
            transactions_counter = 0
            file_total_amount = 0.0
            lines = []
            tz = timezone(self.env.user.tz or 'UTC')
            file_creation_date = datetime.now(tz).strftime("%Y%m%d")
            file_creation_time = datetime.now(tz).strftime("%H%M")
            merchant_id = '0043922558'
            # encabezado o header del archivo
            #header primer campo Registro de Resumen de 1 a 100, pero no se pone datos en principio
            file_header = "{0:<1}".format("0") + \
                          "{0:<8}".format("DEBLIQD") + \
                          "{0:<10}".format(merchant_id) + \
                          "{0:<10}".format("900000    ") + \
                          "{0:<8}".format(file_creation_date) + \
                          "{0:<4}".format(file_creation_time) + \
                          "{0:<1}".format("0") + \
                          "{0:<2}".format("") + \
                          "{0:<55}".format("") + \
                          "{0:<1}".format("*") + "\r"
            # comparamos con 101 por "\r"
            assert len(file_header) == 101, "El registro cabecera debe tener 100 caracteres, pero encontramos: " + \
                                            str(len(file_header)) + " caracteres" + "\n" + \
                                            "Linea con error: " + file_header
            lines.append(file_header)
            file_total_amount = 0.0
            for line in self.bank_line_ids:
                transactions_counter += 1
                file_total_amount += line.amount_currency
                # cuerpo o detalle del archivo invoice_id.partner_id line.partner_id.subscription_id
                # busco el id de la factura para pasarlo en el archivo - identificador del debito: line.partner_id.customer_number
                invoice_id = self.env["account.move"].search([("name", "=", line.communication)], limit=1).id
                subcription_id = self.env["sale.subscription"].search([("partner_id", "=", line.partner_id.id)], limit=1)
                file_body = "{0:<1}".format("1") + \
                            "{0:<16}".format(subcription_id.x_studio_nmero_de_tarjeta) + \
                            "{0:<3}".format("") + \
                            "{0:0>8}".format(invoice_id) + \
                            "{0:<8}".format(datetime.now(tz).strftime("%Y%m%d")) + \
                            "{0:<4}".format("0005") + \
                            '{0:0>15}'.format('{:.2f}'.format(line.amount_currency).replace('.', '')) + \
                            "{0:0>15}".format(line.partner_id.vat) + \
                            "{0:<1}".format("") + "{0:<28}".format("") + "{0:<1}".format("*") + "\r"
                # comparamos con 101 por "\r"
                assert len(file_body) == 101, "El registro detalle debe tener 100 caracteres, pero encontramos: " + \
                                              str(len(file_body)) + " caracteres" + "\n" + \
                                              "Registro: " + line.communication + "\n" + \
                                              "Línea con error: " + file_body + "\n" + "Linea: " + str(line)
            ...
            file_footer = "{0:<1}".format("9") + \
                          ...
                          "{0:<8}".format(file_creation_date) + \
                          "{0:<4}".format(file_creation_time) + \
                          "{0:0>7}".format(transactions_counter) + \
                          "{0:0>15}".format('{:.2f}'.format(file_total_amount).replace('.', '')) + \
                          "{0:<36}".format("") + \
                          "{0:<1}".format("*") + "\r\n"
            # comparamos con 102 por "\r\n"
            assert len(file_footer) == 102, "El registro footer debe tener 100 caracteres, pero encontramos: " + \
                                            str(len(file_footer)) + " caracteres" + "\n" + \
                                            "Linea con error: " + file_footer
            lines.append(file_footer)
        elif self.payment_method_id.code == "Mastercard":
            card_brand_and_type = "DEBLIMC "
            transactions_counter = 0
            file_total_amount = 0.0
            lines = []
            tz = timezone(self.env.user.tz or 'UTC')
            ...
            assert len(file_footer) == 102, "El registro footer debe tener 100 caracteres, pero encontramos: " + \
                                            ...
                                            "Linea con error: " + file_footer
            lines.append(file_footer)
        elif self.payment_method_id.code == "Tuya":
            transactions_counter = 0
            file_total_amount = 0.0
            lines = []
            card_brand_and_type = "TUYA "
            tz = timezone(self.env.user.tz or 'UTC')
            file_creation_date = datetime.now(tz).strftime("%d%m%y")
            file_creation_time = datetime.now(tz).strftime("%H%M")
            company_id = self.env.user.company_id
            company = self.env['res.company'].search([('id', '=', company_id.id)])
            merchant_id = company.num_comer
            # encabezado o 311030013547410206220header del archivo
            #header primer campo Registro de Resumen de 1 a 100, pero no se pone datos en principio
            file_header = "{0:<13}".format(merchant_id) + \
                          "{0:<1}".format("1") + \
                          "{0:<6}".format(file_creation_date) + \
                          "{0:<20}".format("0") + \
                          "{0:<2}".format("0") + \
                          "{0:<3}".format("0") + \
                          "{0:<40}".format("") + "\r"

            # comparamos con 85 por "\r"
            assert len(file_header) == 86, "El registro cabecera debe tener 100 caracteres, pero encontramos: " + \
                                            str(len(file_header)) + " caracteres" + "\n" + \
                                            "Linea con error: " + file_header
            lines.append(file_header)
            file_total_amount = 0.0
            for line in self.bank_line_ids:
                ...
                invoice_id = self.env["account.move"].search([("name", "=", line.communication)], limit=1).id
                subcription_id = self.env["sale.subscription"].search([("partner_id", "=", line.partner_id.id)], limit=1)
                parte_decimal, parte_entera = math.modf(line.amount_currency)
                parte_entera = str(parte_entera).split('.')[0]
                parte_decimal = (str(round(parte_decimal, 2)).split('.')[1][:2]).ljust(2, '0')
                #311030013547420               195222      001001001973     99     1                  @@@
                #3110300135474 2 0               195217      001001001973     99     1                  @@@
                file_body = "{0:<13}".format(merchant_id) + \
                            "{0:<1}".format("2") + \
                            "{0:<16}".format(subcription_id.x_studio_nmero_de_tarjeta) + \
                            "{0:<12}".format(invoice_id) + \
                            "{0:<3}".format("001") + \
                            "{0:<3}".format("001") + \
                            "{0:<2}".format("00") + \
                            '{0:<9}'.format(parte_entera) + \
                            '{0:<2}'.format(parte_decimal) + \
                            "{0:<5}".format("") + \
                            "{0:<1}".format("1") + \
                            "{0:<2}".format("") + \
                            "{0:<16}".format("") + \
                            "{0:<3}".format("@@@") + \
                            "{0:<15}".format("") + "\r"
                # comparamos con 103 por "\r" hasta aca 31-05-2022 FA-X 00011-00002891-FA-B 00006-00002514
                assert len(file_body) == 104, "El registro detalle debe tener 104 caracteres, pero encontramos: " + \
                                              str(len(file_body)) + " caracteres" + "\n" + \
                                              "Registro: " + line.communication + "\n" + \
                                              "Línea con error: " + file_body + "\n" + "Linea: " + str(line)
                lines.append(file_body)
            # cierre o footer del archivo 31103001354749030622000000100000001973.00.9900000000000091000
            parte_decimal, parte_entera = math.modf(file_total_amount)
            file_footer = "{0:<13}".format(merchant_id) + \
                          "{0:<1}".format("9") + \
                          "{0:<6}".format(file_creation_date) + \
                          "{0:0>7}".format(transactions_counter) + \
                          "{0:0>13}".format(parte_entera) + \
                          "{0:0>2}".format(parte_decimal) + \
                          "{0:<3}".format("000") + \
                          "{0:<40}".format("") + "\r\n"
            # comparamos con 85 por "\r\n" 31103001354749080622000000100000001580.00.009999999999990905000
            assert len(file_footer) == 105, "El registro footer debe tener 103 caracteres, pero encontramos: " + \
                                            str(len(file_footer)) + " caracteres" + "\n" + \
                                            "Linea con error: " + file_footer
            lines.append(file_footer)
        return bytes("\n".join(lines), "UTF-8"), card_brand_and_type.strip()
